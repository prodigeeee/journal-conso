...
class _MainNavigationScreenState extends State<MainNavigationScreen>
  ...
  Timer? _syncTimer;
  // Debounce pour le push cloud automatique après chaque sauvegarde locale
  ...
  Future<void> _saveAll() async {
    ...
    // 3. Push cloud automatique avec debounce de 3 secondes
    //    Si plusieurs _saveAll() s'enchaînent (import batch, drag&drop…),
    //    un seul push est effectué 3s après le dernier appel.
    _cloudSyncDebounce?.cancel();
            ...
            return pw.Column(
              ...
              children: [
                              ...
                              child: pw.Column(
                                ...
                                children: [
                                  ...
                                  ...[
                                      ...
                                      .where((m) {
                                        final key =
                                            "${p.id}_${DateFormat('yyyyMMdd').format(currentCalDay)}_$m";
                                        return _contexts.containsKey(key) &&
                                            _contexts[key]!.isNotEmpty;
                                      })
                                      .map((m) {
                                        final key =
                                            "${p.id}_${DateFormat('yyyyMMdd').format(currentCalDay)}_$m";
                ...
                pw.Row(
                  ...
                  children: [
                    pw.Text(
                      "Généré par Alcohol Tracker - Journal Conso",
                      style: pw.TextStyle(
                        fontSize: 7,
                        color: PdfColors.grey500,
                        fontStyle: pw.FontStyle.italic,
      ...
      child: Column(
        ...
        children: [
          ...
          ...[
            ...
            (entry) => _EntranceFadedSlide(
              key: ValueKey(
                "${_selectedDate.toIso8601String()}_${entry.value}",
              ),
              index: entry.key,
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: TweenAnimationBuilder<double>(
                        tween: Tween<double>(
                          begin: 0,
                          end: (currentBac / 1.5).clamp(0, 1),
                        ),
                        duration: const Duration(milliseconds: 1500),
                        curve: Curves.easeOutExpo,
                        builder: (context, value, child) =>
                            _LiquidCircularProgress(
                              value: value,
      ...
      lineTouchData: LineTouchData(
        touchTooltipData: LineTouchTooltipData(
          ...
          getTooltipColor: (spot) =>
              widget.isDarkMode ? const Color(0xFF263238) : Colors.white,
...
class _SaisieSheetState extends State<_SaisieSheet> {
    ...
    if (!_isListening) {
      try {
        ...
        if (available) {
          ...
          try {
            localeId = locales
                .firstWhere((l) => l.localeId.contains('fr'))
                .localeId;
          } catch (e) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text("Dictée vocale non disponible sur cet appareil"),
              ),
            );
          }
        }
  ...
  void initState() {
    ...
    if (widget.existingConso != null) {
      ...
      // Pour les consos saisies après minuit (< 6h), la date stockée est le
      // lendemain du jour logique. On récupère le jour logique pour l'affichage
      // et pour que _handleSave puisse recalculer correctement.
      final d = widget.existingConso!.date;
      final logicalD = d.hour < 6 ? d.subtract(const Duration(days: 1)) : d;
      _selectedDate = DateTime(logicalD.year, logicalD.month, logicalD.day);
    } else {
      _time = TimeOfDay.now();
      final d = widget.date;
                    ...
                    child: Column(
                      ...
                      children: [
                        Container(
                          decoration: BoxDecoration(
                            color: isDark
                                ? const Color(0xFF161A20).withValues(alpha: 0.6)
                            ...
                            children: [
                              ...
                              Expanded(
                                child: _buildWheelSelector(
                                  ...
                                  effectiveAccent,
                                  enabled:
                                      _t != L10n.s('common.soft') || _spinning,
                                ),
                              ),
                            ],
                          ),
  ...
  void _handleSave() {
    ...
    // _selectedDate est normalisé au jour logique (minuit).
    // Si l'heure est < 6h, la date réelle stockée est le lendemain calendaire,
    // ce que belongsToLogicalDay() compensera automatiquement lors de l'affichage.
    DateTime baseDate = _selectedDate;
    if (_time.hour < 6) {
      baseDate = baseDate.add(const Duration(days: 1));
    }
  ...
  Widget _buildWheelSelector(
    ...
    final bool isDark = widget.isDarkMode;
    final Color displayAccent = enabled
        ? accent
                ...
                child: ListWheelScrollView.useDelegate(
                  ...
                  childDelegate: ListWheelChildBuilderDelegate(
                    ...
                    builder: (context, index) {
                      ...
                      String dispNum = display.replaceAll(
                        RegExp(r'[^0-9.]'),
                        '',
                      );
  ...
  Widget _buildPremiumDateTimeBlock(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isDark
              ? const Color(0xFF161A20).withValues(alpha: 0.8)
              : Colors.black.withValues(alpha: 0.03),
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
            color: isDark
...
class _EntranceFadedSlideState extends State<_EntranceFadedSlide>
...
class _LiquidCircularProgress extends StatefulWidget {
  ...
  @override
  State<_LiquidCircularProgress> createState() =>
      _LiquidCircularProgressState();
}

class _LiquidCircularProgressState extends State<_LiquidCircularProgress>
    with SingleTickerProviderStateMixin {
  late AnimationController _ctrl;
    ...
    for (int i = 0; i < 100; i++) {
      ...
      _particles.add(
        _ConfettiParticle(
          x:
              0.3 +
              _rng.nextDouble() * 0.4, // More concentrated center (0.3 to 0.7)
...
class _StreakAuraState extends State<_StreakAura>
    with SingleTickerProviderStateMixin {
  late AnimationController _ctrl;
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: const Color(
                  0xFFFFD700,
                ).withValues(alpha: 0.3 * _ctrl.value * intensity),
